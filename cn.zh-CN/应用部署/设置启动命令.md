...
例如Dockerfile中设置了以下命令，在容器启动时将被第一个执行。
...
|**options快捷设置**：只有**Java环境**为**Dragonwell X**时可以设置。|
|开启微服务性能提升（Wisp 2协程）|开关默认开启，开启后可以提升微服务运行时性能及Thread线程性能。|
|开启应用内存优化（ZGC/G1）|开关默认开启，开启后可以提升GC效率，降低GC时间，适用于GC需要优化、大数据需要提升等场景。|
...
|持久化目录|开启应用加速后需要设置，设置的**持久化目录**必须为NAS存储的目录或者子目录。关于如何设置NAS存储，请参见[设置NAS存储](/cn.zh-CN/应用部署/设置NAS存储.md)。|
