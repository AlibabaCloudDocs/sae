...
某些场景下，SAE为了完成自身的某个功能，需要获取其他云服务的访问权限。例如创建应用时要获取您的VPC等信息，就可以通过SLR获取VPC等产品的访问权限。通过SLR可以更好地配置云服务正常操作所必须的权限，避免误操作带来的风险。SLR的权限策略由关联的云服务定义和使用，您不能修改或删除权限策略，也不能为SLR添加或移除权限。关于SLR的更多详细信息请参见[服务关联角色](/cn.zh-CN/角色管理/服务关联角色.md)。
...
SAE支持SLR的自动创建，当您未创建SLR时，登录[SAE控制台](https://sae.console.aliyun.com)时会弹出**欢迎使用Serverless应用引擎SAE**对话框，单击**确认创建**即可完成SLR的创建。

![SAE创建SLR角色](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/6851377951/p133582.png)

**说明：** 当您未创建SLR时，登录[SAE控制台](https://sae.console.aliyun.com/)会持续显示**欢迎使用Serverless应用引擎SAE**对话框直至您完成SLR的创建。
...
-   如果您在删除SLR后仍需要再次使用SAE，请登录[SAE控制台](https://sae.console.aliyun.com)重新创建SLR角色，详细操作请参见[创建SLR](#section_d0k_noc_31h)。

-   删除应用的具体操作请参见[删除应用](/cn.zh-CN/应用管理/应用生命周期/管理应用生命周期.md)。
-   删除SLR角色的具体操作请参见[删除RAM角色](/cn.zh-CN/角色管理/删除RAM角色.md)。
